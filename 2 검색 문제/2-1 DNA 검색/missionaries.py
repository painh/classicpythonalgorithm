...
class MCState:
    ...
    def __str__(self) -> str:
        boat = "서" if self.boat else "동"
        return f"""서쪽 강둑에는 {self.wm}명의 선교사와 {self.wc}명의 식인종이 있다.
        동쪽 강둑에는 {self.em}명의 선교사와 {self.ec}명의 식인종이 있다.
        배는 {boat}쪽에 있다."""
...
def display_soulution(path: List[MCState]):
    if len(path) == 0:
        return
    ...
    for current_state in path[1:]:
        if current_state.boat:
            print(
                f"{old_state.em - current_state.em}명의 선교사와 {old_state.ec - current_state.ec}명의 식인종이 동쪽 강둑에서 서쪽 강둑으로 갔다.")
        else:
            print(
...
if __name__ == "__main__":
    ...
    soulution : Optional[Node[MCState]] = bfs(
        start, MCState.goal_test, MCState.successors)
    if soulution is None:
        print("답을 찾을 수 없습니다")
    else:
        path: List[MCState] = node_to_path(soulution)
