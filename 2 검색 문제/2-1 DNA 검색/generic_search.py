...
def dfs(initial: T, goal_test: Callable[[T], bool],
        successor: Callable[[T], List[T]]) -> Optional[Node[T]]:
    # frontier 방문하지 않은 곳
...
def astar(initial: T, goal_test: Callable[[T], bool],
          successor: Callable[[T], List[T]], heuristic: Callable[[T], float]) -> Optional[Node[T]]:
    # frontier 방문하지 않은 곳
...
if __name__ == "__main__":
    print(linear_contains([1, 5, 15, 15, 15, 15, 15, 20], 5))
    print(binary_contains(["a", "d", "e", "f", "z"], "f"))
    print(binary_contains(["john", "mark", "ronald", "sarah"], "f"))
